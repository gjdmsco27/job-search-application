# 📱 Job Search App (React Native)

Rapid API를 활용하여 실시간 구직 정보를 검색하고 조회할 수 있는 React Native 애플리케이션입니다.
Expo Router를 기반으로 파일 시스템 라우팅을 구현하였습니다.

## ✨ 주요 기능 (Key Features)

### 1. 기본 기능 (Core Features)
- **홈 화면 (Home):**
  - 사용자 환영 메시지 (개인화된 인사말 및 디자인 강조)
  - 직무 검색 바 (커스텀 Placeholder 적용)
  - **인기 키워드 필터:** `React Native`, `Remote`, `Python` 등 트렌디한 검색어 버튼 제공 (가로 스크롤)
- **인기 채용 공고 (Popular Jobs):** 가로 스크롤 가능한 인기 채용 리스트
- **주변 채용 공고 (Nearby Jobs):** 세로 리스트 형태의 주변 채용 정보
- **상세 페이지 (Job Details):**
  - 채용 공고의 상세 내용 (About, Qualifications, Responsibilities) 탭 구현
  - 회사 정보 및 로고 표시

### 2. 추가 구현 기능 (Custom Features) 🚀
기본 과제 외에 추가로 직접 구현한 기능들입니다:

- **👤 마이 페이지 (My Profile):**
  - 메인 화면 우측 상단 프로필 버튼을 통해 접근
  - 사용자 프로필 이미지(커스텀 스타일링) 및 정보 표시 화면 구현

- **🍔 카테고리 메뉴 (Category Menu):**
  - 좌측 상단 햄버거 버튼(≡) 클릭 시 메뉴 페이지 이동
  - **채용 형태 필터:** `Full-time`, `Part-time`, `Contractor` 버튼 제공 (클릭 시 해당 검색 결과로 즉시 이동)
  - **앱 정보 하단 고정:** 앱 버전, 개발자 정보(Developed by 허은채)를 화면 하단에 배치하여 UI 완성도 향상

- **📤 공유하기 (Share):**
  - 채용 상세 페이지 우측 상단 공유 버튼 활성화
  - `Share` API를 사용하여 외부 앱(카카오톡, 메시지 등)으로 링크 공유 가능

- **👀 더보기 연결 (Show All):**
  - 메인 리스트의 'Show all' 버튼 클릭 시, 해당 카테고리(Popular/Nearby)에 맞는 검색 결과 페이지로 이동하도록 라우팅 연결

### 3. 🎨 UI/UX 및 디테일 개선 (UI/UX Improvements)
사용자 경험을 높이기 위해 세부적인 디테일을 수정하였습니다:

- **상세 페이지 가독성 대폭 개선:** (New!)
  - **텍스트 레이아웃 최적화:** `Qualifications` 및 `Responsibilities` 리스트의 불렛 포인트와 텍스트가 어긋나지 않도록 정렬(`flex-row`) 수정
  - **가독성 향상:** 줄 간격(Line Height)을 넓히고, 텍스트 색상을 눈이 편안한 차분한 회색(`#444262`)으로 조정하여 정보 전달력 강화
  - **디자인 통일:** 불렛 포인트 디자인을 진한 회색(`#6B7280`)으로 변경하여 전체적인 톤 앤 매너 유지

- **메인 헤더 디자인:**
  - 상단 기본 타이틀('Header')을 제거하여 깔끔한 UI 구성
  - 환영 메시지에 **이모지(👋)** 및 **포인트 컬러(#2563EB)** 적용
  - 부제목("지금 바로 수천 개의...")을 추가하여 정보 전달력 강화

- **데이터 안정성 확보 (Mock Data):**
  - API 데이터가 비어있을 경우(About, Qualifications 등), 빈 화면 대신 미리 준비된 **기본 예시 데이터**가 표시되도록 처리하여 시연 시 안정성 확보

## 🛠️ 기술 스택 (Tech Stack)
- **Framework:** React Native / Expo
- **Routing:** Expo Router (v2)
- **Language:** JavaScript
- **API:** Rapid API (JSearch)
- **Styling:** React Native StyleSheet

## 🚀 실행 방법 (How to run)

1. 패키지 설치
   ```bash
   npm install
2. 앱 실행
    Bash
    npx expo start